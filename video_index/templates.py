...
def _base(content: htpy.Node) -> htpy.Element:
    return htpy.html(lang='en')[
        htpy.head[
            ...
            htpy.meta(name='viewport', content='width=device-width, initial-scale=1, shrink-to-fit=no'),
...
def _nav(active_page: str = 'files') -> htpy.Element:
    ...
    for p in pages:
        ...
        page_elements.append(
            htpy.li('.nav-item')[
                htpy.a(cls, href=p.get('href'))[p.get('title')]
            ]
        )
    return htpy.nav('.align-items-center.pt-3.row')[
        htpy.div('.col-12')[
            htpy.ul('.nav.nav-tabs')[page_elements],
        ]
    ]
...
def files_list(files: list[m.File]) -> str:
    ...
    for i, f in enumerate(files):
        if i < 5:
            ...
            cards.append(
                htpy.div('.col.pb-3', hx_include='#card-filters',
...
def index() -> str:
    return htpy.render_node(_base([
        ...
        htpy.form('#card-filters.align-items-center.pt-3.row', hx_target='#video-cards')[
            htpy.div('.col-auto')[
                htpy.input('.form-control.mb-2', hx_post=flask.url_for('files_cards'),
                           hx_trigger='search, keyup changed delay:300ms', name='q', placeholder='Search...',
                           type='search')
            ],
            htpy.div('.col-auto')[
                htpy.div('.form-check.form-switch.mb-2')[
                    htpy.input('#missing-notes-only.form-check-input', hx_post=flask.url_for('files_cards'),
                               name='missing-notes-only', type='checkbox'),
                    htpy.label('.form-check-Label', for_='missing-notes-only')['Only show files missing notes']
                ]
            ]
        ],
        htpy.div('#video-cards.pt-3.row.row-cols-1.row-cols-sm-2.row-cols-lg-3.row-cols-xxl-4',
                 hx_include='#card-filters', hx_post=flask.url_for('files_cards'), hx_trigger='load')
    ]))
...
def locations(locs: list[m.Location]) -> str:
    ...
    return htpy.render_node(_base([
        ...
        htpy.form('.g-1.pt-3.row', action=flask.url_for('locations_add'), method='post')[
            htpy.div('.col-auto')[
                htpy.input('.form-control', name='root-folder', required=True, type='text')
            ],
            htpy.div('.col-auto')[
...
def suffixes(suffix_counts: list[m.SuffixCount]) -> str:
    if suffix_counts:
        content = htpy.table('.align-middle.d-block.table')[
            m.SuffixCount.thead,
            htpy.tbody[[s.tr for s in suffix_counts]]
        ]
    else:
        content = [
